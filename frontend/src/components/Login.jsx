        ...
        <div className="neu-card" style={{ maxWidth: '480px', width: '100%', background: 'var(--card-bg)', position: 'relative', overflow: 'visible' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              <label style={{ fontFamily: '"Archivo Black", sans-serif', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <button 
              ...
              style={{ marginTop: '1rem', background: 'var(--accent-primary)', color: 'var(--text-on-accent)', fontSize: '1.25rem', padding: '1rem', width: '100%', justifyContent: 'center' }}
            >
              {loading ? 'AUTHENTICATING...' : (
                <>
                  <LogIn size={20} style={{ marginRight: '8px' }} /> LOGIN NOW
                </>
              )}
            </button>
          </form>

          <div style={{ marginTop: '2.5rem', textAlign: 'center', paddingTop: '1.5rem', borderTop: '2px dashed var(--border-color)' }}>
            <p style={{ fontFamily: '"Inter", sans-serif', fontWeight: 700 }}>
              New to the terrace?{' '}
              <Link to="/register" style={{ color: 'var(--accent-primary)', textDecoration: 'underline', textDecorationThickness: '3px' }}>
                Create an account
              </Link>
            </p>
          </div>
